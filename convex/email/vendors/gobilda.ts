/**
 * goBILDA Email Parser
 *
 * Parses order confirmations and shipping notifications from goBILDA.
 * goBILDA is a popular FTC vendor known for their channel-based build system.
 *
 * Common email patterns:
 * - Order confirmation: "Your goBILDA Order"
 * - Shipping notification: "Your goBILDA order has shipped"
 */
...
/**
 * Extract order number from goBILDA email
 */
function extractOrderNumber(email: EmailContent): string | undefined {
  const content = email.html || email.text || "";
  const subject = email.subject || "";

  // Try subject first - goBILDA often uses "Order #GB-12345"
  let match = subject.match(/Order\s*#?\s*(GB-?\d+|\d+)/i);
  if (match) return match[1];

  // Try body patterns
  match = content.match(/Order\s*(?:#|Number:?)\s*(GB-?\d+|\d+)/i);
  if (match) return match[1];

  // WooCommerce style order ID
  match = content.match(/order[_-]?id[=:]\s*(\d+)/i);
  if (match) return match[1];
...
/**
 * Extract tracking numbers from goBILDA shipping email
 */
...
export const gobildaParser: VendorParser = {
  vendorId: "gobilda",
  vendorName: "goBILDA",
  domains: ["gobilda.com"],

  canHandle(email: EmailContent): boolean {
    return isFromDomain(email.from, "gobilda.com");
  },
